...
export default function Profile() {
  ...
  const [tab,     setTab]     = useState("edit"); // "edit" | "reviews"
  ...
  const avatarBg = avatarColors[(email.charCodeAt(0)||0) % avatarColors.length];
        ...
        <div className="page-inner">
          ...
          <div style={s.grid}>
            ...
            <div style={s.avatarCard}>
              ...
              <span
                className={`badge ${role === "client" ? "badge-cyan" : "badge-purple"}`}
              ...
              {skillList.length > 0 && (
                <div style={s.skillsWrap}>
                  {skillList.slice(0, 6).map(sk => (
                    <span key={sk} style={s.skillTag}>{sk}</span>
                  ))}
                  {skillList.length > 6 && (
                    <span style={{ ...s.skillTag, background:"rgba(255,255,255,0.06)", color:"var(--text3)" }}>
                      +{skillList.length - 6}
                    </span>
                  )}
                </div>
              ...
              {form.experience && (
                <div style={s.expBadge}>🎯 {form.experience} experience</div>
              )}
            ...
            <div style={s.rightPanel}>
              ...
              <div style={s.tabs}>
                <button
                  onClick={() => setTab("edit")}
                  style={{ ...s.tab, ...(tab==="edit" ? s.tabActive : {}) }}
                >
                  ✏️ Edit Info
                </button>
                {role === "freelancer" && (
                  <button
                    ...
                    style={{ ...s.tab, ...(tab==="reviews" ? s.tabActive : {}) }}
                  >
                    ⭐ My Reviews
                  </button>
                )}
...
const s = {
  ...
  avatarCard:{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:18, padding:"32px 24px", textAlign:"center", display:"flex", flexDirection:"column", alignItems:"center", gap:10 },
  avatar:    { width:80, height:80, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, fontWeight:800, color:"#fff", fontFamily:"'Syne',sans-serif", marginBottom:4, boxShadow:"0 8px 24px rgba(0,0,0,0.3)" },
  ...
  expBadge:  { fontSize:12, color:"var(--text2)", background:"var(--bg3)", padding:"6px 14px", borderRadius:20, border:"1px solid var(--border)" },
  ...
  rightPanel: { display:"flex", flexDirection:"column", gap:0 },
  tabs:       { display:"flex", gap:4, background:"var(--bg3)", borderRadius:"12px 12px 0 0", padding:"6px 6px 0", borderBottom:"1px solid var(--border)" },
  tab:        { padding:"10px 20px", borderRadius:"8px 8px 0 0", fontSize:14, fontWeight:500, border:"none", background:"transparent", color:"var(--text2)", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.2s" },
  tabActive:  { background:"var(--bg2)", color:"var(--text)", fontWeight:600, boxShadow:"0 -2px 0 var(--accent) inset" },

  formCard:  { background:"var(--bg2)", border:"1px solid var(--border)", borderTop:"none", borderRadius:"0 0 18px 18px", padding:"28px 32px" },
